...
{
    [HarmonyPatch(typeof(GameData.CoreLanguage.Collections.DataBaseLanguage), "GetBeverageLang")]
    public class DataBaseLanguageGetBeverageLangPatch
    {
        static void Postfix(int id, ref object __result)
        {
            ...
            string customTranslation = Plugin.GetCustomTranslation("BeveragesLang", id.ToString());
            if (customTranslation != null && __result != null)
            {
                ...
                Plugin.Logger.LogDebug($"Replaced beverage name for ID {id}: {customTranslation}");
            }
        }
    }
}
